...
class pload(object):
    ...
    def ReadGridFile(self, gridfile):
        ...
        **Inputs**:
        ...
        self.x1r = np.zeros(len(self.x1)+1) ; self.x1r[1:] = self.x1 + old_div(self.dx1,2.0) ; self.x1r[0] = self.x1r[1]-self.dx1[0]
        self.x2r = np.zeros(len(self.x2)+1) ; self.x2r[1:] = self.x2 + old_div(self.dx2,2.0) ; self.x2r[0] = self.x2r[1]-self.dx2[0]
        self.x3r = np.zeros(len(self.x3)+1) ; self.x3r[1:] = self.x3 + old_div(self.dx3,2.0) ; self.x3r[0] = self.x3r[1]-self.dx3[0]
        ...
        **Note**:
        ...
        if (self.x1range != None):
            if logr == 0:
                ...
                self.x1range = [log(old_div(self.x1range[0],x1b)),log(old_div(self.x1range[1],x1b))]
            ibeg0 = old_div(min(self.x1range),dx0) ; iend0 = old_div(max(self.x1range),dx0)
            ibeg  = max([ibeg, int(ibeg0*freb[0])]) ; iend = min([iend,int(iend0*freb[0]-1)])
            nx = iend-ibeg+1
        if (self.x2range != None):
            self.x2range = old_div((self.x2range-x2b),ystr)
            jbeg0 = old_div(min(self.x2range),dx0) ; jend0 = old_div(max(self.x2range),dx0)
            jbeg  = max([jbeg, int(jbeg0*freb[0])]) ; jend = min([jend,int(jend0*freb[0]-1)])
            ny = jend-jbeg+1
        if (self.x3range != None):
            self.x3range = old_div((self.x3range-x3b),zstr)
            kbeg0 = old_div(min(self.x3range),dx0) ; kend0 = old_div(max(self.x3range),dx0)
        ...
        # Create the xr arrays containing the edges positions
        # Useful for pcolormesh which should use those
        x1r = np.zeros(len(x1)+1) ; x1r[1:] = x1 + old_div(dx1,2.0) ; x1r[0] = x1r[1]-dx1[0]
        x2r = np.zeros(len(x2)+1) ; x2r[1:] = x2 + old_div(dx2,2.0) ; x2r[0] = x2r[1]-dx2[0]
        x3r = np.zeros(len(x3)+1) ; x3r[1:] = x3 + old_div(dx3,2.0) ; x3r[0] = x3r[1]-dx3[0]
        ...
        for i in range(il+1):
            ...
            for j in range(nbox): # loop on all boxes of a given level
                ...
                if ((ib > iend) or (ie < ibeg) or \
                    ...
                    cib0 = old_div((ib0-ib),freb[i]) ; cie0 = old_div((ie0-ib),freb[i])
                    cjb0 = old_div((jb0-jb),freb[i]) ; cje0 = old_div((je0-jb),freb[i])
                    ckb0 = old_div((kb0-kb),freb[i]) ; cke0 = old_div((ke0-kb),freb[i])
...
class Tools(object):
    ...
    def deriv(self,Y,X=None):
        ...
        **Outputs:**
        ...
        DfDx = np.roll(Yarr,1) * (old_div(x12, (x01*x02))) + Yarr * (old_div(1.,x12) - old_div(1.,x01)) - np.roll(Yarr,-1) * (old_div(x01, (x02 * x12)))
    ...
    def Grad(self,phi,x1,x2,dx1,dx2,polar=False):
        ...
        **Outputs:**
        ...
        for i in range(n1):
            scrh1 = phi[i,:]
            grad_phi[i,:,1] = old_div(self.deriv(scrh1,x2),h2[i,:])
        for j in range(n2):
            scrh2 = phi[:,j]
    ...
    def sph2cyl(self,D,Dx,rphi=None,theta0=None):
        ...
        **Usage:**
        ...
        nR1=find(Rs_copy<rx[0]-old_div(ddr,1.5))
        nR2=find(Rs_copy>rN+old_div(ddr,1.5))
        nTh1=find(Th_copy>th[-1]+old_div(ddth,1.5))
        nTh2=find(Th_copy<th[0]-old_div(ddth,1.5))
    ...
    def congrid(self, a, newdims, method='linear', centre=False, minusone=False):
        ...
        **Outputs:**
        ...
        if method == 'neighbour':
            ...
            deltas = old_div((np.asarray(old) - m1), (newdims - m1))
            newcoords_tr *= deltas
                ...
                if ((Slice-box[inds+'b'])*(box[inds+'e']-Slice) >= 0):
                    if (geom == 'CARTESIAN'):
                        ...
                        xb = np.concatenate([
                                ...
                                x1*np.cos(np.linspace(y0,y1,num=int(old_div(abs(y0-y1),dn)) )),\
                                [x1*np.cos(y1),x0*np.cos(y1)],\
                                x0*np.cos(np.linspace(y1,y0,num=int(old_div(abs(y0-y1),dn))))])
                        yb = np.concatenate([
                                [x0*np.sin(y0),x1*np.sin(y0)],\
                                x1*np.sin(np.linspace(y0,y1,num=int(old_div(abs(y0-y1),dn)))),\
                                [x1*np.sin(y1),x0*np.sin(y1)],\
                                x0*np.sin(np.linspace(y1,y0,num=int(old_div(abs(y0-y1),dn))))])
